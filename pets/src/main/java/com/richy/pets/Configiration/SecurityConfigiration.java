...
public class SecurityConfigiration {
    ...
    return  http.csrf(customize-> customize.disable()).authorizeHttpRequests(auth->auth.requestMatchers("/homePage").permitAll().anyRequest().authenticated()).formLogin(Customizer.withDefaults()).build();
       ...
       //loginPage method is which endpoint to get verified from the login page, will make a custom loginPage page with Url /loginPage
        //loginProcessingUrl is which login form you want to intercept to get password and username from whether its the generated login form or your custom one. It automatically intercepts the generated login form if dont include this method( might need to include loginForm method for this to happen though)df
